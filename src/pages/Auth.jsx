...
export function Auth() {
  ...
  useEffect(() => {
    document.title = isRegister ? 'Crear cuenta | DATE' : 'Iniciar Sesión | DATE';
  }, [isRegister]);
      ...
      <form className='bg-[#faf8f4] border border-[#ddd8cf] p-8 rounded-2xl shadow-[0_10px_30px_rgba(0,0,0,0.1)] w-110 flex flex-col gap-5 overflow-hidden'>
        ...
        <div className='flex flex-col'>
          ...
          <p className='text-sm opacity-60 mb-4'>
            {isRegister ? 'Regístrate para comenzar a usar la plataforma.' : 'Ingresa para gestionar tu agenda y punto de venta.'}
          </p>
        </div>
        ...
        <div className='relative h-65'>
          <div
            className={`absolute w-full transition-all duration-500 ${
              isRegister ? 'opacity-0 -translate-x-10 scale-95 blur-sm pointer-events-none' : 'opacity-100 translate-x-0 scale-100'
            }`}
          >
            <LoginForm />
          </div>
          ...
          <div
            className={`absolute w-full transition-all duration-500 ${
              isRegister ? 'opacity-100 translate-x-0 scale-100' : 'opacity-0 translate-x-10 scale-95 blur-sm pointer-events-none'
            }`}
          >
            <RegisterForm />
          </div>
